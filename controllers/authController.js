...
exports.register = async (req,res) => {
    try{
        ...
        const existingUser = await User.findOne({where:{loginuser}});
            if(existingUser){
                return res.status(400).json({menssage:'Login already exists'});
            }
        const hashedPassword =  await bcrypt.hash(password,10);
        const user = await User.create({username,loginuser,password:hashedPassword,active:true});
        res.status(201).json({menssage:'user created sucessfully', user});
...
exports.login = async (req,res) => {
    try{
        ...
        if (user && await bcrypt.compare(password, user.password)){
            ...
            res.status(401).send({menssage: 'Invalid credentials or inactive user'});
        }
        //return res.status(200).json({menssage: 'OK LOGIN'});
    } catch(error){
        res.status(500).send(error.message);
    } 
};                                                                                                                                                                                             